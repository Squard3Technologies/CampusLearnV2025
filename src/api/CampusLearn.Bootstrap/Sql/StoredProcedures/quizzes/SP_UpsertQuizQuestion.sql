...
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM [CampusLearnDB].[dbo].[User] WHERE Id = @UserId)
        BEGIN
            SET @Status = 0;
            SET @StatusCode = 404;
    ...
    SELECT @Status AS [Status], @StatusCode AS [StatusCode], @StatusMessage AS [StatusMessage], @QuestionId AS [Body];
END;